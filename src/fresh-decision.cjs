...
async function decideFresh({observe,decide,position,onDiscard,onServiceError=()=>{},onCorrelation=()=>{},signal,isActive=()=>true,now=Date.now,wait=(ms=250)=>delay(ms,undefined,{signal}),maxAttempts=3}) {
  for(let attempt=1;attempt<=maxAttempts;attempt++) {
    signal?.throwIfAborted();
    if(!isActive())return null;
    const correlation={attempt,observationId:randomUUID(),requestId:null};
    onCorrelation({...correlation});
    trace.event('inference.attempt',correlation);
    const state=await trace.run(correlation,observe);
    signal?.throwIfAborted();
    if(!isActive())return null;
    const started=now();
    correlation.requestId=randomUUID();onCorrelation({...correlation});
    let result;
    try { result=await trace.run(correlation,()=>decide(state,correlation)); }
    catch(error) {
      signal?.throwIfAborted();
      if(!isActive())return null;
      ...
      onServiceError({...correlation,kind:'api-error',httpStatus:timedOut?null:error.status,errorType:timedOut?'timeout':'http',delayMs,retry});
    ...
    if(isFresh(state.position,current,elapsedMs))return {state,result,freshness,correlation};
    onDiscard({...correlation,state,...result,freshness,outcome:'Not executed: observation changed or response exceeded 5 seconds'});
