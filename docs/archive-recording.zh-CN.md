...
适用范围：本分支的新版 `schemaVersion: 2` 归档。每次启动 dashboard 创建一组文件，不是每轮创建新文件；新旧格式以文件内容为准。

本分支控制方式：Lumber 默认高层动作，可按原有配置切换 direct 模式。本次同步只调整归档关联，沿用分支既有机器人身份和功能。

不适用于本分支的功能不会产生相应事件。例如 RCON 记录只存在于启用 RCON 的版本。
...
| `rconCommandId` | 启用 RCON 功能的版本中，一条手动命令的关联 ID；不会伪装成模型决策 |
